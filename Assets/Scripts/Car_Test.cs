...
public class Car_Test : WaypointCar
{
    ...
    [Space]
    [SerializeField] private LayerMask rammingMask;
    [SerializeField] private float rammingSpeedThreshold = 17.5f;
    [SerializeField] private float rammingDetectionDistance = 3f;
    [SerializeField] private int rammingSegmentsPerSide = 4;
    [Space]
    ...
    private Vector3 GetLookAheadPointOnTrack(Vector3 currentTrackPoint, float lookAheadDistance)
    {
        ...
        return currentTrackPoint + direction * lookAheadDistance + waypointOffset;
    }
    ...
    private void CalculateStearing()
    {
        ...
        float anglePerSegment = 45f / collisionSegmentsPerSide;
        ...
        //Ramming
        float ramStearingAngle = 0;

        float ramStartAngle = 65f;
        float ramAnglePerSegment = 40f / rammingSegmentsPerSide;

        for (int i = 0; i <= rammingSegmentsPerSide; i++)
        {
            if (GetRammingCollision(rightFrontalPosition, ramStartAngle + ramAnglePerSegment * i, rammingDetectionDistance, out CollisionInfo _, true))
            {
                ramStearingAngle += 10f;
            }
            if (GetRammingCollision(leftFrontalPosition, -ramStartAngle - ramAnglePerSegment * i, rammingDetectionDistance, out CollisionInfo _, true))
            {
                ramStearingAngle -= 10f;
            }
        }

        if (speed < rammingSpeedThreshold) { ramStearingAngle = 0f; }

        //Steering
        float stearingAngle = Mathf.Clamp((signedAngleToLookAhead + collisionStearingAngle + ramStearingAngle) / maxStearingError, -1f, 1f);
    ...
    private bool GetRammingCollision(Vector3 origin, float angleInDegrees, float distance, out CollisionInfo collisionInfo, bool drawDebug = true)
    {
        collisionInfo = new CollisionInfo();
        Vector3 angleVector = RotateVector(transform.forward, angleInDegrees);
        bool collidedWithCar = Physics.Raycast(origin, angleVector, out RaycastHit carRaycastHit, distance, rammingMask);

        if (drawDebug && collidedWithCar) { Debug.DrawRay(origin, angleVector.normalized * distance, Color.yellow); }
